...
| --- | --- | --- |
| `/root/.claude/` | Claude credentials, MCP token caches, projects state | rw, persistent |
| `/root/.claude/skills/` | Skills directory (overlay on the auth volume) | ro |
| `/etc/claude-agent/mcp.json.tmpl` | MCP server declarations with `${VAR}` placeholders | ro |
| `/etc/supercronic/crontab` | Job schedule | ro |

The entrypoint renders `mcp.json.tmpl` to `/root/.mcp.json` (project-scope, picked up because `WORKDIR` is `/root`) using `envsubst`, then execs supercronic.
...
Follow the OAuth prompt from any browser. The credential persists at `/root/.claude/.credentials.json` on the auth volume and survives image rebuilds.
...
docker run --rm -it \
    ...
    -v "$PWD/test/skills:/root/.claude/skills:ro" \
